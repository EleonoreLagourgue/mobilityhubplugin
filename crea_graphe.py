...
class BuildGraphAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterVectorLayer(self.INPUT, "Réseau"))
        self.addParameter(QgsProcessingParameterField(self.DIRECTION_FIELD, "Colonne direction", parentLayerParameterName=self.INPUT))
        self.addParameter(QgsProcessingParameterString(self.VALUE_FORWARD, "Sens direct"))
        self.addParameter(QgsProcessingParameterString(self.VALUE_BACKWARD, "Sens inverse"))
        self.addParameter(QgsProcessingParameterString(self.VALUE_BOTH, "Double sens"))
        self.addParameter(QgsProcessingParameterEnum(self.DEFAULT_DIRECTION,
                                                 "Direction par défaut",
                                                 list(self.DIRECTIONS.keys()),
                                                 defaultValue=2))

        self.addParameter(QgsProcessingParameterField(self.SPEED_FIELD, "Colonne vitesse", parentLayerParameterName=self.INPUT))
        self.addParameter(QgsProcessingParameterString(self.DEFAULT_SPEED, "Vitesse par défaut", defaultValue=50))
       
        
        self.addParameter(QgsProcessingParameterNumber(self.TOLERANCE, "Tolérance topologique", defaultValue=0.0))
        ...
        self.addParameter(QgsProcessingParameterFeatureSink(self.LIGNES, "Couche linéaire du graphe"))
        self.addParameter(QgsProcessingParameterFeatureSink(self.NOEUDS, "Couche des noeuds du graphe"))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        graph = crea_graphe(gdf_route, colonne_direction, value_forward, value_backward, value_both)
